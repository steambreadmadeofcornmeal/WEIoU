...
def symmetric_kl_laplace(mu_p, mu_q, b_p, b_q, eps=1e-6):
    """
    计算两个 Laplace 分布的 symmetric KL 散度。
    """
    b_p = torch.clamp(b_p, min=eps)
    b_q = torch.clamp(b_q, min=eps)
    diff_mu = torch.abs(mu_p - mu_q)
    term1 = diff_mu * 0.5 * (1.0 / b_q + 1.0 / b_p)
    term2 = 0.5 * (b_q / b_p + b_p / b_q)
    sym_kl = term1 + term2 - 1.0
    return torch.clamp(sym_kl, min=0.0, max=100.0)  # 防止极端值
...
def bbox_iou(box1, box2, xywh=True, GIoU=False, DIoU=False, CIoU=False, EIoU=False, SIoU=False, ShapeIoU=False, PIoU=False, PIoU2=False, ProbIoU=False, MCIoU=False, LEpIoU=False, eps=1e-7, scale=0.0, Lambda=1.3, num_samples=100, Focal=False,gamma=0.5):
    """
    ...
    Args:
        ...
        ProbIoU (bool, optional): If True, calculate Probabilistic IoU. Defaults to False.
        MCIoU (bool, optional): If True, calculate Monte Carlo IoU based on EIoU. Defaults to False.
        eps (float, optional): A small value to avoid division by zero. Defaults to 1e-7.
        scale (float, optional): Scale factor for certain IoUs. Defaults to 0.0.
        Lambda (float, optional): Lambda for PIoU2. Defaults to 1.3.
        num_samples (int, optional): Number of samples for MCIoU. Defaults to 100.
    ...
    # IoU
    iou = inter / union
    if CIoU or DIoU or GIoU or EIoU or SIoU or ShapeIoU or PIoU or PIoU2 or ProbIoU or MCIoU or LEpIoU:
        cw = b1_x2.maximum(b2_x2) - b1_x1.minimum(b2_x1)  # convex (smallest enclosing box) width
        ch = b1_y2.maximum(b2_y2) - b1_y1.minimum(b2_y1)  # convex height
        if CIoU or DIoU or EIoU or SIoU or PIoU or PIoU2 or ShapeIoU or ProbIoU or MCIoU or LEpIoU:  # Distance or Complete IoU https://arxiv.org/abs/1911.08287v1
            ...
            if CIoU:  # https://github.com/Zzh-tju/DIoU-SSD-pytorch/blob/master/utils/box/box_utils.py#L47
                ...
                if LEpIoU:
                    # 添加 Laplace KL 惩罚 (normalized)
                    b_x1 = w1 / math.sqrt(24.0) + eps
                    b_y1 = h1 / math.sqrt(24.0) + eps
                    b_w1 = w1 / math.sqrt(24.0) + eps
                    ...
                    kl_term = (kl_x + kl_y + kl_w + kl_h) / 4.0
                    
                    # 归一化 KL (e.g., exp(-kl) 作为相似度加权)
                    norm_kl = torch.exp(-kl_term)  # [0,1] 范围
                    
                    # 新相似度
                    return (eiou * norm_kl).clamp(0.0, 1.0)
                elif Focal:
                    return eiou, torch.pow(iou, gamma) # 原版Focal_EIou
                    # return eiou, torch.pow(iou, 1 - iou) 
                else:
                    return eiou # EIoU
            elif SIoU:
                ...
                # Model bounding boxes as Gaussians
                cx1 = (b1_x1 + b1_x2) / 2
                cy1 = (b1_y1 + b1_y2) / 2
                cx2 = (b2_x1 + b2_x2) / 2
                cy2 = (b2_y1 + b2_y2) / 2
                ...
                # KLD(G1 || G2)
                trace_term1 = sx1_2 / (sx2_2 + eps) + sy1_2 / (sy2_2 + eps)
                dist_term1 = (cx1 - cx2) ** 2 / (sx2_2 + eps) + (cy1 - cy2) ** 2 / (sy2_2 + eps)
                log_term1 = torch.log((sx2_2 * sy2_2) / (sx1_2 * sy1_2 + eps))
                kld1 = 0.5 * (trace_term1 + dist_term1 - 2 + log_term1)

                # KLD(G2 || G1)
                trace_term2 = sx2_2 / (sx1_2 + eps) + sy2_2 / (sy1_2 + eps)
                dist_term2 = (cx1 - cx2) ** 2 / (sx1_2 + eps) + (cy1 - cy2) ** 2 / (sy1_2 + eps)
                log_term2 = torch.log((sx1_2 * sy1_2) / (sx2_2 * sy2_2 + eps))
                kld2 = 0.5 * (trace_term2 + dist_term2 - 2 + log_term2)

                kld_sym = (kld1 + kld2) / 2
                prob_iou = torch.exp(-kld_sym)
                return prob_iou
            elif MCIoU:
                b1_x1 = b1_x1.squeeze(-1)  # (1,)
                b1_y1 = b1_y1.squeeze(-1)
                b1_x2 = b1_x2.squeeze(-1)
                b1_y2 = b1_y2.squeeze(-1)
                b2_x1 = b2_x1.squeeze(-1)  # (n,)
                b2_y1 = b2_y1.squeeze(-1)
                b2_x2 = b2_x2.squeeze(-1)
                b2_y2 = b2_y2.squeeze(-1)
                w1 = w1.squeeze(-1)
                h1 = h1.squeeze(-1)
                w2 = w2.squeeze(-1)
                h2 = h2.squeeze(-1)
                iou_clamped = iou.clamp(0, 1)
                # sigma = (1 - iou_clamped) * scale * torch.sqrt(c2 + eps)
                sigma = (1 - iou_clamped) * torch.sqrt(c2 + eps)

                # Sample for box2
                n = box2.shape[0]
                noise = torch.randn((n, num_samples), device=sigma.device) * sigma[:, None]  # (n, num_samples), 广播 sigma (n,1)
    
                b2_x1_s = b2_x1.unsqueeze(1).expand(-1, num_samples) + noise
                b2_y1_s = b2_y1.unsqueeze(1).expand(-1, num_samples) + noise
                b2_x2_s = b2_x2.unsqueeze(1).expand(-1, num_samples) + noise
                b2_y2_s = b2_y2.unsqueeze(1).expand(-1, num_samples) + noise

                # Ensure valid boxes by taking min/max
                x1_s = torch.min(b2_x1_s, b2_x2_s)
                x2_s = torch.max(b2_x1_s, b2_x2_s)
                y1_s = torch.min(b2_y1_s, b2_y2_s)
                y2_s = torch.max(b2_y1_s, b2_y2_s)
                b1_x1_s = b1_x1.expand(n).unsqueeze(1).expand(-1, num_samples) + noise#x1  # 如果需要噪声+ noise 
                b1_y1_s = b1_y1.expand(n).unsqueeze(1).expand(-1, num_samples) + noise#y1 
                b1_x2_s = b1_x2.expand(n).unsqueeze(1).expand(-1, num_samples) + noise#x2 
                b1_y2_s = b1_y2.expand(n).unsqueeze(1).expand(-1, num_samples) + noise#y2

                # Compute inter_s
                inter_s = (torch.min(x2_s, b1_x2_s) - torch.max(x1_s, b1_x1_s)).clamp(0) * \
                        (torch.min(y2_s, b1_y2_s) - torch.max(y1_s, b1_y1_s)).clamp(0)

                # Compute union_s
                w2_s = x2_s - x1_s + eps
                h2_s = y2_s - y1_s + eps
                w1_s = w1.expand(n)[:, None].expand(-1, num_samples)
                h1_s = h1.expand(n)[:, None].expand(-1, num_samples)
                union_s = w1_s * h1_s + w2_s * h2_s - inter_s + eps

                # iou_s
                iou_s = inter_s / union_s

                # Compute EIoU terms for samples
                cw_s = torch.max(x2_s, b1_x2_s) - torch.min(x1_s, b1_x1_s)
                ch_s = torch.max(y2_s, b1_y2_s) - torch.min(y1_s, b1_y1_s)
                c2_s = cw_s ** 2 + ch_s ** 2 + eps
                rho2_s = ((x1_s + x2_s - b1_x1_s - b1_x2_s) ** 2 + (y1_s + y2_s - b1_y1_s - b1_y2_s) ** 2) / 4
                rho_w2_s = ((x2_s - x1_s) - (b1_x2_s - b1_x1_s)) ** 2
                rho_h2_s = ((y2_s - y1_s) - (b1_y2_s - b1_y1_s)) ** 2
                cw2_s = cw_s ** 2 + eps
                ch2_s = ch_s ** 2 + eps

                # EIoU for samples
                eiou_s = iou_s - (rho2_s / c2_s + rho_w2_s / cw2_s + rho_h2_s / ch2_s)

                # Average over samples
                mciou = eiou_s.mean(dim=1)
                return mciou
