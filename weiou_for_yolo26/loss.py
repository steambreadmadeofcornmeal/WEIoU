...
def probiou_loss(pred, target, eps=1e-4, mode='l1',gamma=0.5):
    ...
    #focal-probiou
    # modulating_factor = bc.pow(gamma) #原教旨主义focal
    # modulating_factor = (1.0 - bc).pow(1.0 - bc)
...
class BboxLoss(nn.Module):
    ...
    def __init__(self, reg_max: int = 16):
        ...
        if self.use_wiseiou:
            self.wiou_loss = WiseIouLoss(ltype='EIoU', monotonous=True, inner_iou=False, focaler_iou=False)
    ...
    def forward(
        ...
        if self.use_wiseiou:
            ...
            iou = bbox_iou(pred_bboxes[fg_mask], target_bboxes[fg_mask], xywh=False, CIoU=True, focal=False, gamma=5.0)
